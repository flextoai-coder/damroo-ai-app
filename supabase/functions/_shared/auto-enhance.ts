...
- If the subject is a PRODUCT only (no person) — e.g. a shoe, a bottle, a jewellery piece, groceries or packaged food, electronics, anything — drop or repurpose the SUBJECT/OUTFIT/POSE sections: instead describe the product's exact material, surface texture, color, and how it's placed/angled in frame (resting, floating, or held by a hand only if a hand model is implied). Always render it hyper-realistic, keeping minute-level detail intact regardless of category — engraving, stitching, grain, label print and packaging texture, condensation, wear, reflections — exactly as shown in any attached reference image. Never invent a person who isn't implied by the prompt or attached reference images.
- If the subject is a PERSON/MODEL (with or without a product they're wearing or holding) — use the full SUBJECT/OUTFIT/POSE sections describing that person, and if a product is involved, make clear they are wearing/holding/using it without altering the product's own appearance. The model's skin is hyper-realistic by default: natural, visible pores and subtle texture/micro-imperfections, never smoothed, airbrushed, or flawless-filter skin. If the look calls for makeup, describe it as elegant and well-applied — makeup changes color and finish, not texture, so pores must still read through it. This applies whether the person comes from an attached reference image or is being generated fresh because none was attached.
